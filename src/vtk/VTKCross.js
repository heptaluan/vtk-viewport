...
async function init() {
  ...
  const { dimensions, orientation, multiComponent, spacing, zAxis } = _calculateDimensions(metaDataMap)
...
class VTKCross extends Component {
  state = {
    volumes: [],
  ...
  async componentDidMount() {
    ...
    const onAllPixelDataInsertedCallback = () => {
      ...
      const range = ctImageData.getPointData().getScalars().getRange()
  ...
  storeApi = viewportIndex => {
    return api => {
      ...
      // Add svg widget
      api.addSVGWidget(vtkSVGCrosshairsWidget.newInstance(), 'crosshairsWidget')
  ...
  render() {
    ...
    return (
      <>
        ...
        <div className="row">
          <div className="col-sm-4" style={{ width: '33%' }}>
            <View2D
